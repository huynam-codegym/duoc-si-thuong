...
- **Số điện thoại 0988 283 415** lưu ở `src/lib/site.ts` (một nơi duy nhất). Nút liên hệ là component `ZaloButton`: bấm vào mở **Zalo** của số này (`https://zalo.me/0988283415`, mở tab mới, trên điện thoại mở ứng dụng Zalo nếu đã cài). Có ở trang chủ, trang Giới thiệu và chân trang. Chủ website chọn Zalo thay cho gọi điện thường; liên kết `tel:` vẫn còn trong `site.ts` (`phoneHref`) nhưng hiện không nút nào dùng. Nếu số điện thoại đổi mà không còn dùng Zalo, sửa `zaloHref`. Đổi số thì chỉ sửa `site.ts` và ảnh bìa (số cũng nằm trong ảnh, phải sửa alt ở `src/pages/index.astro`).
