...
- **Giá khuyến mãi (tháng 9/2026, chủ website yêu cầu, theo mẫu Long Châu gửi):** trường `originalPrice` trong frontmatter sản phẩm (không bắt buộc) là **giá gốc trước khuyến mãi**; trường `price` vẫn luôn là **giá thật sự tính tiền** (giỏ hàng, tạm tính, bộ lọc/sắp xếp giá ở `/tim-kiem/` đều dùng `price` như cũ, không cần sửa gì thêm). Có điền `originalPrice` và giá này **lớn hơn** `price` thì trang tự tính phần trăm giảm (hàm `discountPercent()` trong `src/lib/products.ts`, làm tròn) và hiện: **thẻ sản phẩm** — nhãn đỏ "-X%" đè góc trên-phải ảnh (`.sale-badge-card`) + giá gốc gạch ngang dưới giá bán; **trang chi tiết** — giá gốc gạch ngang kèm nhãn đỏ "-X%" nằm ngang ngay dưới giá lớn. Không điền `originalPrice` (hoặc điền nhỏ hơn/bằng `price`) thì không hiện gì thêm, y như trước. Màu đỏ (`#dc2626`) chọn riêng cho nhãn khuyến mãi, khác hẳn màu vàng/hổ phách của khung cảnh báo y khoa (`--warn-bg`) để không gây nhầm lẫn giữa "đang giảm giá" và "lưu ý sức khỏe". **Chưa làm** (không có dữ liệu/chính sách thật): huy hiệu "Khuyến mãi được áp dụng" kèm hạn kết thúc kiểu phiếu giảm giá, đếm ngược flash sale, mã giảm giá — chỉ thêm khi chủ website cho biết chương trình khuyến mãi cụ thể (ngày bắt đầu/kết thúc).
