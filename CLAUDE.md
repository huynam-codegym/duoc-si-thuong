...
- **Biểu tượng nhỏ trong menu thả xuống:** mỗi nhóm có trường `icon` trong `categories.ts`/`departments.ts`, kiểu `IconName` (`src/lib/icon-names.ts`). Biểu tượng là **icon đường nét tự vẽ** (component `src/components/GroupIcon.astro`, SVG nét mảnh màu xanh lá, khớp màu thương hiệu), **không dùng emoji nữa** (chủ website yêu cầu tháng 9/2026, vì emoji hiện mỗi máy/điện thoại một kiểu, không đồng bộ). Biểu tượng chỉ để trang trí (`aria-hidden`), chữ bên cạnh vẫn là tên nhóm. Dùng ở khắp dự án: menu thả xuống, menu lớn của từng khu bán hàng, menu bên trái khu bán hàng, thẻ danh mục sản phẩm ở trang chủ (`<GroupIcon badge />`, có khung tròn), tiêu đề trang nhóm sản phẩm, và dải cam kết ở trang chủ (`doctor`, `chat`, `book`).
  ...
  - **Bảng ánh xạ hiện tại** (một icon có thể dùng cho nhiều nhóm cùng ý nghĩa): `salad` nguyên tắc ăn uống; `bone` canxi/xương khớp; `droplet` sắt/đường huyết/tiểu đường; `sun` vitamin D; `egg` vitamin B12; `shell` kẽm; `wave` i-ốt; `citrus` vitamin C/vitamin-khoáng chất; `leaf` acid folic; `bacteria` kháng sinh; `thermometer` giảm đau hạ sốt; `capsule` kháng viêm; `stomach` dạ dày/tiêu hóa; `wind` ho cảm cúm; `flower` dị ứng/phụ nữ mẹ bầu; `heart` tim mạch; `shield` dùng thuốc an toàn/đề kháng; `eye` mắt; `brain` thần kinh não; `sparkle` làm đẹp; `baby` trẻ em; `liver` gan - mật; `book`/`cart` mục "Xem tất cả"; `doctor`/`chat` dải cam kết trang chủ; `grid` tiêu đề "Danh mục [khu]"; `target` tiêu đề "Sản phẩm theo nhu cầu"; `calendar` tiêu đề "Bệnh thường gặp theo mùa" (3 icon sau chỉ dùng cho tiêu đề mục ở trang chủ, không phải nhóm sản phẩm/bài viết).
...
- **Nhóm sản phẩm của từng khu (tháng 9/2026, tôi đặt tạm theo kiểu hiệu thuốc trực tuyến, chủ website có thể bớt hoặc đổi):**
  - Thực phẩm chức năng (15 nhóm, thêm **sinh lý - nội tiết tố** tháng 9/2026, icon `flower`, cho sản phẩm tiền mãn kinh/mãn kinh): vitamin & khoáng chất, **vitamin tổng hợp** (thêm tháng 9/2026, theo mẫu Long Châu gửi, dùng chung icon `citrus` với "Vitamin & khoáng chất"), đề kháng & miễn dịch, mắt & thị lực, tiêu hóa, **gan - mật** (thêm tháng 9/2026, icon `liver` riêng — vẽ mới vì chưa có icon nào biểu thị gan), thần kinh & trí nhớ, hỗ trợ làm đẹp, đường huyết, tim mạch, xương khớp, **canxi & vitamin D** (thêm tháng 9/2026, chủ website yêu cầu tách riêng khỏi "Vitamin & khoáng chất" theo mẫu Long Châu gửi — cùng dùng icon `bone` với "Xương khớp"), phụ nữ & mẹ bầu, trẻ em. Các nhóm thêm sau đều là nhóm **cùng cấp** với "Vitamin & khoáng chất" (site chỉ có 2 cấp khu/nhóm, chưa có nhóm con cấp 2) — breadcrumb sản phẩm vì vậy chỉ có 1 tên nhóm ở giữa (`Trang chủ / Thực phẩm chức năng / <tên nhóm> / <tên sản phẩm>`), không lồng nhiều cấp như ảnh mẫu đôi khi cho thấy.
  - **Lưu ý về nhóm "Vitamin tổng hợp":** sản phẩm Pregnacare Plus Omega-3 (dành riêng cho phụ nữ mang thai/cho con bú) đang xếp vào nhóm này theo đúng đường dẫn chủ website chỉ định (tháng 9/2026), dù về công dụng sẽ hợp với nhóm "Phụ nữ & mẹ bầu" (đã có sẵn) hơn. Chưa tự chuyển vì đó là đường dẫn chủ website đưa ra rõ ràng — nếu muốn chuyển sang nhóm "Phụ nữ & mẹ bầu" chỉ cần đổi `group` trong frontmatter sản phẩm.
...
- **Giao diện kiểu cửa hàng (tham khảo kiểu hiệu thuốc trực tuyến, không sao chép thương hiệu hay nội dung của website nào):** đầu trang gồm hàng logo + **ô tìm kiếm lớn** (gửi tới `/tim-kiem/?q=từ-khóa`, viền khi bấm vào là quầng sáng nhẹ chứ không phải viền cứng) + nút **Giỏ hàng**, hàng menu có 4 mục khu bán hàng rồi các chuyên mục kiến thức và Giới thiệu. Cả 7 mục cùng một kiểu chữ đen, không có mục nào nổi bật hơn mục khác (chủ website yêu cầu tháng 9/2026, theo mẫu Long Châu gửi — trước đó "Thực phẩm chức năng" có nền pill xanh đậm mặc định, nay đã bỏ); rê chuột (hoặc bấm mũi tên) vào mục nào thì chữ mục đó đổi màu xanh và có gạch chân, đồng thời mở danh sách nhóm/menu lớn của mục đó — **áp dụng như nhau cho cả 7 mục**, xem `.site-nav a`, `.site-nav .has-sub:hover > .mega-menu`/`.submenu` trong `global.css`. Mục "Tìm kiếm" cũ trong menu đã bỏ vì có ô tìm kiếm.
  - **Bẫy từng gặp:** trước đó menu lớn (`.mega-menu`, dùng cho 4 khu bán hàng) chỉ mở khi rê chuột vào đúng khu đầu tiên, vì CSS `:hover` viết nhắm riêng lớp `.nav-shop` (lớp chỉ gắn cho khu đầu). 3 khu còn lại vẫn bấm mũi tên mở được (dùng lớp `.open` chung) nhưng rê chuột thì không, nên dễ bị coi nhầm là "3 khu kia không có danh mục". Đã sửa bằng cách đổi luật `:hover` sang áp dụng chung cho mọi `.has-sub` (khớp với cách chuyên mục kiến thức vốn đã làm đúng từ đầu). Khi thêm hộp thả xuống mới ở thanh menu, luôn kiểm tra **cả bấm mũi tên lẫn rê chuột** trên từng mục, không chỉ mục đầu tiên. Vì có 7 mục nên thanh menu tự xuống dòng ở hầu hết màn hình (khung nội dung rộng tối đa 68rem), đó là bình thường, không phải lỗi. Trang chủ (tháng 9/2026, bố cục có tham khảo trang chủ Long Châu chủ website gửi nhưng chỉ dùng những phần hợp với site tĩnh và nội dung có thật, xem "Những phần không làm theo mẫu" bên dưới): ảnh bìa, dải cam kết (chỉ nêu điều có thật: dược sĩ đại học tư vấn, nhắn Zalo, bài có nguồn), **dải "Liên hệ nhanh"** (6 ô nhỏ: Tư vấn dược sĩ, Thực phẩm chức năng, Ăn uống & dinh dưỡng, Thuốc, Giỏ hàng, Giới thiệu — chỉ trỏ tới trang có thật, không thêm ô "Tìm nhà thuốc"/"Đơn của tôi" vì chưa có). **Thứ tự các mục sau đó (đổi tháng 9/2026, chủ website yêu cầu đưa khu bán hàng lên đầu):** (1) với **mỗi khu đã có sản phẩm thật** hiện tối đa 12 sản phẩm nổi bật ("[Tên khu]", ví dụ "Thực phẩm chức năng") **rồi mới tới** "Danh mục [tên khu]" (lưới icon nhóm) — chủ website đổi thứ tự này tháng 9/2026, trước đó "Danh mục" đứng trên (khu chưa có sản phẩm thì tự ẩn cả hai phần, không hiện mục trống); (2) "Sản phẩm theo nhu cầu"; (3) "Bệnh thường gặp theo mùa" (trước tháng 9/2026 mục này từng đứng ở vị trí (1), nay đổi xuống vị trí (3), đổi chỗ với khu bán hàng); (4) "Video ngắn" (nếu có); (5) "Góc sức khỏe" (chuyên mục và bài mới). **Mỗi tiêu đề mục (`.section-title`) đều có icon đường nét nhỏ bên trái tên mục** (chủ website yêu cầu tháng 9/2026): "Danh mục [khu]" dùng `grid`, "[Tên khu]" (ví dụ "Thực phẩm chức năng") dùng `cart`, "Sản phẩm theo nhu cầu" dùng `target`, "Bệnh thường gặp theo mùa" dùng `calendar` — xem bảng ánh xạ icon ở mục "Nhóm con và menu thả xuống" phía trên. **Trên máy tính (≥77rem, cùng mốc "máy tính" dùng chung toàn site):** khối sản phẩm này (và khối mỗi tab trong "Sản phẩm theo nhu cầu") hiện thành **lưới 4 cột**, xem hết tối đa 12 sản phẩm ngay không cần bấm (chủ website yêu cầu tháng 9/2026, theo mẫu Long Châu gửi — trước đó là cuộn ngang, phải bấm mũi tên mới thấy hết từ sản phẩm thứ 5). **Điện thoại/máy tính bảng vẫn cuộn ngang + bấm mũi tên** như cũ (đánh dấu `.carousel-wide`, lớp `.carousel-track` đổi từ `display:flex;overflow-x:auto` sang `display:grid;grid-template-columns:repeat(4,1fr)` chỉ trong `@media (min-width: 77rem)`). Khối hẹp bên trong mỗi tab của "Bệnh thường gặp theo mùa" (`.season-products .carousel`, chỉ tối đa 4 sản phẩm) **không** đổi vì đã đủ hẹp, không có lớp `.carousel-wide`. Không thêm cam kết như "chính hãng 100%", "giao nhanh" khi chưa có thông tin từ chủ website.
...
- **Trang chi tiết sản phẩm:** đường dẫn "Trang chủ / tên khu / nhóm / tên sản phẩm"; bảng thông tin (thương hiệu, xuất xứ, dạng bào chế, thành phần, số công bố, xác nhận quảng cáo, chỉ hiện dòng nào có dữ liệu); **chọn số lượng** rồi "Mua ngay" hoặc "Thêm vào giỏ"; phần mô tả có **mục lục bên trái** (tự lấy từ các tiêu đề `##` trong nội dung, nên dùng các mục Mô tả sản phẩm, Công dụng, Cách dùng, Lưu ý, Bảo quản) và nút **Kích thước chữ: Mặc định / Lớn hơn** (nhớ lựa chọn trên máy người đọc). Chưa có: chọn đơn vị tính, nhiều ảnh (khác gallery đã có), đánh giá, "Tìm nhà thuốc", thanh mua hàng cố định khi cuộn, chính sách đổi trả và vận chuyển (chỉ thêm khi nhà thuốc có chính sách thật). **Không chép giá, đánh giá, huy hiệu "Chính hãng", ảnh hay đoạn mô tả nguyên văn từ website khác** (bản quyền và có thể sai với hồ sơ công bố của mình); viết lại từ nhãn và tài liệu nhà phân phối, câu về công dụng ghi rõ "theo nhà sản xuất". Bỏ các câu kiểu "đã được chứng minh", "giảm nguy cơ...", "hiệu quả", "lý tưởng", so sánh với sản phẩm khác. Xem ví dụ và ghi chú ở `docs/ghi-chu-kiem-duyet-san-pham.md`.
- **Nhãn nhóm trên thẻ sản phẩm** (`.product-card .tag`, ví dụ "Canxi & Vitamin D"): luôn **1 dòng**, tên dài quá thì cắt kèm "…" (không xuống 2 dòng làm thẻ cao lệch nhau); dưới 30rem (điện thoại) chữ nhỏ lại còn 0.66rem — chủ website yêu cầu tháng 9/2026.
- **Giá khuyến mãi (tháng 9/2026, chủ website yêu cầu, theo mẫu Long Châu gửi):** trường `originalPrice` trong frontmatter sản phẩm (không bắt buộc) là **giá gốc trước khuyến mãi**; trường `price` vẫn luôn là **giá thật sự tính tiền** (tạm tính giỏ hàng, bộ lọc/sắp xếp giá ở `/tim-kiem/` đều dùng `price` như cũ, không cần sửa gì thêm). Có điền `originalPrice` và giá này **lớn hơn** `price` thì trang tự tính phần trăm giảm (hàm `discountPercent()` trong `src/lib/products.ts`, làm tròn) và hiện nhãn đỏ "-X%" + giá gốc gạch ngang (class dùng chung `.sale-badge`/`.product-price-original`) ở **CẢ 5 chỗ** hiện giá trên site — nhớ kiểm tra đủ cả 5 khi sửa liên quan đến giá:
  1. **Thẻ sản phẩm** (`ProductCard.astro`) — nhãn đè góc trên-phải ảnh (`.sale-badge-card`) + giá gốc gạch ngang dưới giá bán.
  2. **Trang chi tiết sản phẩm** (`[department]/[slug].astro`) — giá gốc gạch ngang kèm nhãn nằm ngang ngay dưới giá lớn.
  3. **Khay "Chọn số lượng" trên điện thoại** (`#cartSheet` trong `Header.astro`, mở khi bấm "Chọn mua" ở thẻ sản phẩm) — xem mục "Giỏ hàng và đặt hàng" bên dưới.
  4. **Khay xem nhanh giỏ hàng trên máy tính** (`#cartDropdown`, rê chuột vào biểu tượng giỏ) — `renderCartDropdown()` tự thêm dòng `.product-price-original` nếu sản phẩm trong giỏ có `originalPrice`.
  5. **Trang `/gio-hang/`** — mỗi dòng sản phẩm tự thêm nhãn "-X%" cạnh giá và dòng giá gốc gạch ngang nếu có.
  
  Để (3)(4)(5) hiện đúng, **`originalPrice` phải được LƯU VÀO GIỎ HÀNG** cùng lúc với `price` (không chỉ dùng để hiện tạm thời) — xem `CartItem.originalPrice` (không bắt buộc) trong `src/lib/cart.ts`, và mọi nút "Thêm vào giỏ"/"Mua ngay"/"Chọn mua" đều phải có `data-original-price` (chỉ điền khi thật sự đang khuyến mãi, để trống thì không có gì đổi khác). Giỏ hàng vẫn **tính tiền dựa trên `price`** như cũ, `originalPrice` trong giỏ chỉ để hiện lại, không ảnh hưởng `cartTotal()`. Không điền `originalPrice` (hoặc điền nhỏ hơn/bằng `price`) thì không hiện gì thêm ở bất kỳ chỗ nào trong 5 chỗ trên, y như trước khi có tính năng này. Màu đỏ (`#dc2626`) chọn riêng cho nhãn khuyến mãi, khác hẳn màu vàng/hổ phách của khung cảnh báo y khoa (`--warn-bg`) để không gây nhầm lẫn giữa "đang giảm giá" và "lưu ý sức khỏe". **Chưa làm** (không có dữ liệu/chính sách thật): huy hiệu "Khuyến mãi được áp dụng" kèm hạn kết thúc kiểu phiếu giảm giá, đếm ngược flash sale, mã giảm giá — chỉ thêm khi chủ website cho biết chương trình khuyến mãi cụ thể (ngày bắt đầu/kết thúc).
...
- **Giỏ hàng và đặt hàng** (`src/lib/cart.ts`, `src/pages/gio-hang.astro`): nút "Thêm vào giỏ" ở thẻ sản phẩm và trang chi tiết, "Mua ngay" (thêm rồi sang giỏ). Giỏ **gộp chung sản phẩm từ cả 4 khu**, lưu ở **trình duyệt của khách** (localStorage, mỗi lần tối đa 20 cái/sản phẩm), biểu tượng giỏ ở đầu trang có số lượng. Trang `/gio-hang/` cho đổi số lượng, xóa, xem tạm tính (sản phẩm "Liên hệ" chưa tính giá), rồi điền họ tên, số điện thoại (10 chữ số hoặc +84), địa chỉ, ghi chú và ô đồng ý.
  - **Điện thoại/máy tính bảng:** bấm "Chọn mua" ở thẻ sản phẩm (không có sẵn ô chọn số lượng như trang chi tiết) mở **khay "Chọn số lượng"** trượt lên từ dưới (`#cartSheet` trong `Header.astro`) — có sản phẩm khuyến mãi thì khay cũng tự hiện giá gốc gạch ngang + nhãn "-X%" giống thẻ sản phẩm (đọc `data-original-price` trên nút, tự tính % giảm ngay trong script thay vì import `discountPercent()` từ `lib/products.ts` — file đó có `import ... from 'astro:content'` nên import vào script chạy trình duyệt sẽ lỗi build `ServerOnlyModule`, đúng bẫy đã ghi ở mục `foldVietnamese`/`text.ts` phía trên). **Sau khi bấm "Thêm vào giỏ" (cả từ khay này lẫn nút "Thêm vào giỏ" ở trang chi tiết), trang tự cuộn mượt lên đầu trang** (dùng lại hàm `waitScrollTop()` — trước đó hàm này chỉ dùng cho hiệu ứng bay ảnh vào giỏ trên máy tính) để khách thấy ngay biểu tượng giỏ hàng vừa cập nhật số lượng, kèm thông báo dạng chữ (`showToast`) — chủ website yêu cầu tháng 9/2026 vì trước đó chỉ có thông báo chữ, khách phải tự cuộn lên mới thấy giỏ hàng đổi số. Trang giỏ hàng không hiện `ProductNotice` (vì giỏ có thể lẫn sản phẩm nhiều khu, không chọn được cảnh báo nào là đúng); cảnh báo riêng từng sản phẩm đã hiện ở trang chi tiết trước khi thêm vào giỏ. **Chưa có tài khoản khách, chưa có thanh toán trực tuyến, chưa có trang quản lý đơn**; văn bản nói nhà thuốc sẽ xác nhận giá, phí giao hàng và cách thanh toán trước khi giao (chủ website cần xác nhận đúng với thực tế). Có hai chế độ gửi đơn, chọn qua `orderEndpoint` trong `src/lib/site.ts`:
...
- **Video ngắn giới thiệu sản phẩm** (không bắt buộc, thêm tháng 9/2026): khác với "Video ngắn" ở trang chủ (liên kết ra Facebook/YouTube, mở tab mới, xem mục "Video ngắn" phía trên) — video sản phẩm là **file tự tải lên, phát ngay trên trang** bằng thẻ `<video>` chuẩn của trình duyệt (không nhúng mã bên thứ ba). Đặt file trong `public/videos/<tên-sản-phẩm>/`, điền frontmatter `video` (đường dẫn bắt đầu bằng `/videos/...`, build báo lỗi nếu không đúng dạng) và `videoThumbnail` (ảnh đại diện/poster, bắt buộc đi kèm — không tự trích được khung hình từ file video). Có video thì video hiện là **lựa chọn đầu tiên và MẶC ĐỊNH đang mở** trong khung ảnh chính khi vào trang (ảnh sản phẩm lùi thành lựa chọn kế tiếp trong dải ảnh nhỏ, bấm vào mới đổi sang xem ảnh) — khác với hành vi ban đầu (video chỉ là 1 lựa chọn trong dải ảnh nhỏ, mặc định vẫn hiện ảnh). Bấm ảnh nào trong dải ảnh nhỏ thì video tự dừng; hiệu ứng kính lúp phóng to (rê chuột) tự tắt khi đang xem video. Xem `[department]/[slug].astro` (biến `hasVideo`) và `content.config.ts`.
  - **Trạng thái tháng 9/2026:** đang gắn **tạm 1 video mẫu** (không phải video thật, do chủ website gửi để xem thử giao diện) vào sản phẩm Cốm vi sinh Lacto Biomin Gold+ để xem trực tiếp trên web — cần thay bằng video thật của sản phẩm hoặc gỡ bỏ (xóa 2 dòng `video`/`videoThumbnail` và file trong `public/videos/`) khi chủ website xác nhận xong.
- **Trạng thái tháng 9/2026:** chỉ khu Thực phẩm chức năng có sản phẩm thật — 20 sản phẩm đã đăng: Blackmores Omega Double (giá tạm 450.000đ, ảnh còn là ảnh giữ chỗ), Cốm vi sinh Lacto Biomin Gold+, Feroglobin B12 viên, Osteocare viên, Siro Feroglobin B12, Siro Osteocare, Liveril (nhóm "Gan - Mật", số công bố 8719/2019/ĐKSP), Pregnacare Plus Omega-3 (nhóm "Vitamin tổng hợp", dual pack viên nén + viên nang Omega-3 cho mẹ bầu/mẹ cho con bú, số công bố 9184/2019/ĐKSP), Pregnacare Max (nhóm "Vitamin tổng hợp", bản nâng cao của Pregnacare Plus — có thêm canxi và acid folic dạng L-Methylfolate, số công bố 1475/2024/ĐKSP, **sản phẩm khuyến mãi đầu tiên của site**: giá 495.000đ, giá gốc 550.000đ, tự hiện nhãn "-10%"), Visionace Original (nhóm "Mắt & thị lực", hỗ trợ thị lực, 432.000đ, **chưa điền số công bố** vì ảnh nhãn quá mờ — xem ghi chú), Pregnacare Breast-feeding (nhóm "Vitamin tổng hợp", dual pack cho mẹ cho con bú, 712.000đ, số công bố 10645/2019/ĐKSP), và 3 sản phẩm **Doppelherz (Đức, nhập bởi Công ty CP Mastertran, KHÔNG có video, chỉ ảnh)**: Aktiv Vital Pregna (nhóm "Vitamin tổng hợp", 375.000đ), Aktiv Anti Stress (nhóm "Thần kinh & trí nhớ", 349.000đ, công dụng ghi đúng nhãn là giảm căng thẳng/tập trung, không ghi "cải thiện trí nhớ"), Aktiv Omega-3 + Folic acid + B6 + B12 (nhóm "Vitamin tổng hợp", 335.000đ) — ảnh chỉ dùng ảnh hộp/vỉ/nhãn sạch, đã LOẠI các banner có logo Nhà thuốc Long Châu (xem ghi chú) — và **6 sản phẩm Doppelherz đợt 2** (cũng không video, banner tự thiết kế): Aktiv Liver Complex (Gan - Mật, 395.000đ), Aktiv-Meno (nhóm mới "Sinh lý - Nội tiết tố", 411.000đ), Aktiv Magnesium + Calcium + D3 (Canxi & Vitamin D, 375.000đ), Aktiv Coenzyme Q10 (Tim mạch, 375.000đ, số công bố 8117/2019/ĐKSP do chủ website xác nhận), Kinder Omega-3 Syrup (Thần kinh & trí nhớ, 535.000đ, số công bố 2876/2018/ĐKSP do chủ website xác nhận, không đưa câu ADHD trên nhãn vào bài), Kinder Optima Mini-Tabs (Đề kháng & miễn dịch, 506.000đ) — 10 sản phẩm Vitabiotics ở trên (Anh), nhập bởi VE Pharma, nội dung lấy từ ảnh nhãn chủ website gửi, xem `docs/ghi-chu-kiem-duyet-san-pham.md` — cùng **1 sản phẩm cho trẻ em đang ở dạng nháp chờ duyệt** (Siro Wellbaby Multi-vitamin Liquid, `draft: true` dù đã có giá 412.000đ, vì ảnh nhãn phụ tiếng Việt gửi tới mờ không đọc rõ số công bố sản phẩm và đây là sản phẩm cho trẻ từ 4 tháng tuổi — cần dược sĩ xác nhận kỹ trước khi đăng, xem ghi chú) và 1 sản phẩm mẫu (`draft: true`, chỉ thấy khi `npm run dev`). Ba khu còn lại (Dược mỹ phẩm, Chăm sóc cá nhân, Thiết bị y tế) **chưa có sản phẩm nào**, hiện "Sản phẩm đang được cập nhật"; menu và trang nhóm đã có sẵn, chỉ cần thêm bài sản phẩm khi chủ website gửi thông tin.
