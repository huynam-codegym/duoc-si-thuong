...
- **Trang `/tim-kiem/` tự dựng bằng JS API của Pagefind** (`tim-kiem.astro`), **không dùng khung `PagefindUI` đóng gói sẵn nữa** (bản trước dùng `PagefindUI`, đã bỏ) — vì khung đó tự vẽ luôn cả ô nhập từ khóa (trùng với ô ở đầu trang) và không tách riêng được một mục lọc thành hàng chọn kiểu khác. Cấu trúc:
  ...
  - Hàng **"Tìm kiếm theo"** (Tất cả / Sản phẩm / Bài viết sức khỏe) là nhóm radio riêng ngoài cùng, ứng với bộ lọc `Loại nội dung`, không nằm trong danh sách lọc bên trái.
  - Bộ lọc bên trái (`Chuyên mục`, `Khu`, `Nhóm`, `Giá`) dựng thủ công từ `pagefind.filters()` (danh sách đầy đủ, dùng để hiện ngay lúc đầu) và `search.filters` của mỗi lần `pagefind.search()` (số lượng cập nhật theo đúng từ khóa/bộ lọc đang chọn). Trên điện thoại/máy tính bảng, cả khối lọc nằm trong một `<details>` "Bộ lọc" đóng theo mặc định; trên máy tính (≥56rem) CSS ép luôn hiện, không cần bấm.
  - Kết quả hiện ảnh đại diện (Pagefind tự lấy từ `<img>` đầu tiên trong `data-pagefind-body`, không cần khai báo thêm), tiêu đề, đoạn trích.
  - **Bẫy đã gặp:** `pagefind.search(term)` **lượt gọi đầu tiên** của một phiên trang thường trả `result.filters` RỖNG (`{}`) dù gọi lại y hệt ngay sau đó lại có dữ liệu đầy đủ — có vẻ Pagefind tải chỉ mục lọc lười (tách file riêng), lượt đầu chưa kịp tải xong. Nếu chỉ dựa vào `search.filters` để dựng danh sách lọc, khung lọc sẽ trống trơn ở lần tìm đầu tiên. Đã sửa bằng cách gọi thêm `pagefind.filters()` (không phụ thuộc từ khóa) song song lúc tải trang để CHẮC CHẮN có dữ liệu dựng khung lọc ngay, đồng thời `renderFilters()` bỏ qua (giữ nguyên khung cũ) nếu dữ liệu trả về rỗng thay vì xóa trắng khung đang có.
...
- **Menu dạng ngăn kéo trên điện thoại và máy tính bảng (dưới 64rem, ví dụ iPhone, Samsung, iPad đứng)** (chủ website yêu cầu tháng 9/2026, theo mẫu Long Châu gửi, không sao chép giao diện của họ): thanh menu ngang **ẩn hẳn**, thay bằng **nút 3 gạch** ở đầu trang (`#mobile-menu-btn`, cạnh logo). Bấm vào mở khung `#mobile-drawer` trượt ra từ bên trái (rộng tối đa 23rem), có nền mờ phía sau, đóng bằng nút ✕, bấm ra ngoài hoặc nhấn Esc. Bên trong liệt kê đủ 4 khu bán hàng, 2 chuyên mục bài viết và Giới thiệu; mục nào có nhóm con thì bấm mũi tên để mở/đóng kiểu accordion (không hiện khung xem trước sản phẩm như menu lớn trên máy tính, chỉ danh sách tên nhóm kèm icon, để gọn cho màn hình nhỏ). Cuối khung có nút "Nhắn Zalo tư vấn". Ở máy tính bảng nằm ngang hoặc màn hình rộng hơn (≥64rem) quay lại dùng thanh menu ngang bình thường, không có nút 3 gạch. Đổi ngưỡng 64rem thì sửa cả hai nơi: CSS (`@media (max-width: 63.99rem)` và `@media (min-width: 64rem)`) và không cần sửa gì ở JS.
  ...
  - **Tên thương hiệu cạnh logo** (`.brand-name`, chủ website yêu cầu tháng 9/2026): giữ **một dòng** ("Dược Sĩ Thương") ở mọi màn hình, kể cả điện thoại hẹp — không xuống 2 dòng, không ẩn (trừ màn hình cực hẹp ~320px, xem bên dưới). Để vẫn vừa một hàng cùng nút 3 gạch, dưới 30rem: logo nhỏ lại (`2.2rem`), chữ hơi nhỏ lại (`1.1rem`), và **giỏ hàng thu gọn thành chỉ icon** (bỏ viền/nền/chữ "Giỏ hàng", số lượng hiện đè lên góc icon thay vì nằm cạnh, giống nút giỏ hàng của Long Châu trên điện thoại). Markup vẫn là hai `<span>` cách nhau một dấu cách trong `.brand-name` (để trình đọc màn hình đọc đúng cụm "Dược Sĩ Thương"); CSS mặc định `display:inline-flex` (hàng ngang) — **không** còn quy tắc đổi `flex-direction` sang cột nữa. Màn hình cực hẹp (`max-width: 21rem`, khoảng 320px, điện thoại đời rất cũ) mới ẩn hẳn `.brand-name`, chỉ còn logo tròn, vì dù đã thu gọn hết mức vẫn không đủ chỗ. Đổi tên thương hiệu thì sửa cả hai nơi dùng chữ "Dược Sĩ"/"Thương" (đầu trang `Header.astro` và khung ngăn kéo `.mobile-drawer-brand`, khung sau vẫn giữ một dòng cỡ chữ bình thường vì đủ chỗ).
