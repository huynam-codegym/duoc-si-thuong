...
src/
  ...
  components/         # Header, Footer, Disclaimer, PostCard, ZaloButton (nút "Gọi ngay qua Zalo")
...
- **Số điện thoại 0988 283 415** lưu ở `src/lib/site.ts` (một nơi duy nhất). Nút liên hệ là component `ZaloButton`: bấm vào mở **Zalo** của số này (`https://zalo.me/0988283415`, mở tab mới, trên điện thoại mở ứng dụng Zalo nếu đã cài). Có ở trang chủ, trang Giới thiệu và chân trang. Ngoài ra khối "0988 283 415 | Liên hệ ngay" **nằm trong ảnh bìa** cũng bấm được: có một liên kết trong suốt `.banner-cta` phủ lên đúng vị trí đó (trong `src/pages/index.astro`, CSS ở `src/styles/global.css`, vị trí tính theo % của ảnh gốc 2048x762: trái 21.3%, rộng 38.6%, tâm dọc 81.2%, cao 13.6%). **Khi đổi ảnh bìa phải đo lại các con số này** cho khớp khối liên hệ mới. Chủ website chọn Zalo thay cho gọi điện thường; liên kết `tel:` vẫn còn trong `site.ts` (`phoneHref`) nhưng hiện không nút nào dùng. Nếu số điện thoại đổi mà không còn dùng Zalo, sửa `zaloHref`. Đổi số thì chỉ sửa `site.ts` và ảnh bìa (số cũng nằm trong ảnh, phải sửa alt ở `src/pages/index.astro`).
